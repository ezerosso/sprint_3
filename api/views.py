...
def feedback():
    ...
    if request.method == 'POST':
        ...
        try:
            # Attempt to safely evaluate the string to a Python dictionary
            report_data = ast.literal_eval(report)

            # Validate report data
            if not all(key in report_data for key in ('filename', 'prediction', 'score')):
            ...
            # Validate data types
            if not isinstance(filename, str) or not isinstance(prediction, str) or not isinstance(score, (float, int)):
                raise ValueError("Invalid data types in report")

            # Store the original string to a file
            feedback_filepath = settings.FEEDBACK_FILEPATH
            with open(feedback_filepath, 'a') as feedback_file:
                feedback_file.write(report + '\n')

        except (ValueError, SyntaxError) as e:
            raise BadRequest(f"Invalid report format: {e}")
